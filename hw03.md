...
\`\`{r} \# having trouble graphing because ggplot2 doesn't group the way that dplyr does change %&gt;% filter(gdpchange &gt;= 0, lifechange &lt; 0) %&gt;% ggplot(aes(gdpchange, lifechange)) + geom\_bar(aes(stat = identity, position = dodge)) \`\`\`
